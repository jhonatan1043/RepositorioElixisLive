Public Class FormCarteraCXP
    ...
    Private Sub llenardgv(pCodigo As DateTime)
        ...
        params.Add(pCodigo)
        Generales.llenarTabla(Consultas.CARTERA_CXP_CUENTAS_CARGAR, params, dtCartera)
        ...
        Else
            For i = 0 To dgvCartera.Rows.Count - 2
                ...
                If dgvCartera.Rows(i).Cells("cuenta").Value.ToString = "" Then
                    EstiloMensajes.mostrarMensajeAdvertencia("El tercero " + dgvCartera.Rows(i).Cells("Tercero").Value.ToString + ", no tiene cuenta configurada")
                    Return False
                End If
            Next
        End If
    ...
    Private Sub cargarDatos(pCodigo As String)
        ...
        Generales.llenarTabla(Consultas.CARTERA_CXP_CARGAR, params, dt)
        Textsigla.Text = Constantes.SIGLA_SALDOS_INICIALES
        ...
        For Each drFila As DataRow In dtCartera.Rows
            If drFila.Item("Factura").ToString <> "" Then
                ...
                If String.IsNullOrEmpty(identificador) Then
                    drCuenta.Item("comprobante") = FuncionesContables.verificarConsecutivo(codigoDocumento)
                Else
    ...
    Private Sub dgvcartera_CellDoubleClick(sender As Object, e As DataGridViewCellEventArgs) Handles dgvCartera.CellDoubleClick
        Try
            ...
            If (dgvCartera.Rows(dgvCartera.CurrentCell.RowIndex).Cells("Factura").Selected = True) And dtCartera.Rows(dgvCartera.CurrentCell.RowIndex).Item("Estado").ToString = "" Then
                Generales.busquedaItems(Consultas.FACTURAS_CXP_BUSCAR, Nothing, TitulosForm.BUSQUEDA_CAJA_BANCO, dgvCartera, dtCartera, 0, 4, 0, 0)
